...
use MM\Meros\Crm\App\Integrations\ExternalModels\SalesforceContact;
...
class SalesforceContacts extends DynamicChoiceSource {
    public string $source = 'salesforce_contacts';

    protected string $label = 'Salesforce Contacts';

    protected string $description = 'Query Salesforce Contact records.';
    ...
    protected array $configFields = [
        ...
        [
            ...
            'default' => 20,
            'min' => 1,
            'helpText' => 'Maximum number of contact results returned per request.',
        ],
        [
            'key' => 'valueField',
            'label' => 'Value Field',
        ...
        [
            ...
            'type' => 'text',
            'default' => '',
            'helpText' => 'Only include contacts linked to this Account Id.',
        ],
        [
            'key' => 'recordTypeId',
            ...
            'type' => 'text',
            'default' => '',
            'helpText' => 'Only include contacts for this Record Type Id.',
        ],
        [
            'key' => 'advancedQuery',
    ...
    public function resolve(\WP_REST_Request $request): array {
        ...
        $accountId = trim(sanitize_text_field((string) ($request->get_param('accountId') ?: '')));
        ...
        $selectFields = array_values(array_unique(['Id', 'Name', 'FirstName', 'LastName', 'Email', $valueField]));

        $query = SalesforceContact::init($connection)
            ->select($selectFields)
            ->limit($queryLimit);
        ...
        return array_slice(array_values(array_filter(array_map(function ($record) use ($valueField) {
            if (!is_array($record)) {
                return null;
            }
            ...
            $label = $name !== '' ? $name : ($fullName !== '' ? $fullName : $value);
